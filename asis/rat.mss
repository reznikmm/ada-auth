...
@Comment{$Date: 2006/09/26 05:15:07 $}
...
code. Such code analysis capability has in general been under leveraged in
software organizations; but for the Ada language in particular, it can greatly
enhance the development process. Code analysis automation can harness the
excellent software engineering features of Ada to facilitate code
comprehension, high reliability, and high quality of the software product. The
following text presents some motivational background.

@LabeledSubClause{.1	Definition }
...
@LabeledSubClause{.2	Applicability }
...
@LabeledSubClause{.3	Motivation }


Over the years, a wide-ranging set of commercial code analysis tools has become
increasingly available [11]; examples of such tools include:
...
@LabeledSubClause{.1	Code parsers }
...
@LabeledSubClause{.3	LRM-interface }
...
@LabeledSubClause{.5	Benefits of ASIS standard }
...
@LabeledSubClause{.1	Design goals }
...
* ASIS should supplement weak compile time type checking of element values by run-time use of ASIS_Inappropriate_Element Exception.
...
@LabeledSubClause{.2	Major changes from ASIS for ISO 8652:1987}
...
@LabeledSubClause{.3	Essence of Ada and ASIS }


There are some fundamental differences between a language definition (i.e., the definition of Ada) and the interface definition of a tool, such as ASIS. Clearly the Ada language definition, as described in the Ada Reference Manual, and ASIS are different in nature and have different aims.
...
On the other hand, ASIS is more like the requirements definition for a tool which provides information about a set of Ada software components, which may or may not make up a complete partition. Moreover, as a tool, ASIS provides management facilities which go beyond what is needed in a language definition (e.g., ASIS needs terminology for designating the then-part and the else-part of an if statement).
...
@LabeledSubClause{.1	Ada environment and compilation units }
...
The Ada Reference Manual 10.1.4(1) says: "Each compilation unit submitted to the compiler is compiled in the context of an environment declarative_part (or simply, an environment), which is a conceptual declarative_part that forms the outermost declarative region of the context of any compilation. At run time, an environment forms the declarative_part of the body of the environment task of a partition". And the next paragraph says: "The declarative_items of the environment are library_items appearing in an order such that there are no forward semantic dependencies." Reference Manual 10.1.4(5) then says: "When a compilation unit is compiled, all compilation units upon which it depends semantically shall already exist in the environment;..."

At a first look, there seems to be a contradiction between Reference Manual 10.1.4(1,2) which defines an environment as a declarative_part and says that an environment contains library_items, but not compilation_units, and Reference Manual 10.1.4(5) which speaks about the existence of compilation units in the environment.

But this apparent "contradiction" is resolved in Reference Manual 10.1.1(9), which says: "The term compilation unit is used to refer to a compilation_unit. When the meaning is clear from context, the term is also used to refer to the library_item of a compilation_unit or to the proper_body of a subunit (that is, the compilation_unit without the context_clause and without the separate (parent_unit_name))."

This means the Ada Reference Manual defines compilation_unit as a syntactical category, and at the same time it defines "compilation unit" (without underscore and typed in regular font) as another technical term, which can denote either one of the syntactical categories "compilation_unit", "library_item" or "proper_body". ASIS is aware of this subtle difference and consistently use the term "compilation unit". In ASIS "Compilation_Unit" is the name of a type. Objects of this type are always compilation_units (except if they are nonexistent, unknown, or null).

The purpose of the Ada concept of an environment is threefold. It allows Ada to define what is a legal compilation unit, what is a legal partition and what is the behavior of a partition at run-time. Strictly speaking, an Ada environment contains only library_items; indeed, context clauses do not make sense inside of a declarative part, and subunits are enclosed in their parent unit in place of the corresponding stub.

@LabeledSubClause{.2	ASIS context and inconsistency }

ASIS has to deal with sets of compilation units. To avoid any confusion, it therefore introduces a new term, the concept of a context, which stands for a set of compilation_units maintained by an Ada compilation system. A context has a state, consisting of the compilation units belonging to it, and the configuration pragmas applying to them, but also depending on the history of compiler runs, or other actions taken by the compilation system. Thus, the ASIS concept of a context resembles closely an Ada environment when used as a dynamic concept like in the Ada Reference Manual 10.1.4(3), which says: "The mechanisms for creating an environment and for adding and replacing compilation units within an environment are implementation-defined."

Ada does not deal with arbitrary sets of compilation units as a whole, but ASIS does.

EXAMPLE	Suppose we first compile a package P, then a procedure A withing P, then we modify P, recompile it, and finally compile the procedure B which also is withing P; we also suppose that all compiler runs are successful (i.e., do not detect compilation errors):
...
As far as the legality of compilation units is the concern, the environments can be distinguished as: P alone, P together with A, P' alone, and P' together with B. It could well be that compiling A with P' yields a compilation error. Nevertheless, all these environments contain only consistent unit sets following Reference Manual 10.1.4 (5), which says: "When a compilation unit is compiled, all compilation units upon which it depends semantically shall already exist in the environment; the set of these compilation units shall be consistent in the sense that the new compilation unit shall not semantically depend (directly or indirectly) on two different versions of the same compilation unit, nor on an earlier version of itself."

Only when the results of these compilations are used to build a partition, the inconsistency of depending on two versions for P can be detected by the implementation following Reference Manual 10.2 (27), which says: "The implementation shall ensure that all compilation units included in a partition are consistent with one another, and are legal according to the rules of the language."
...
On the contrary, it is clearly desirable for ASIS to deal with the previously described situation as a whole, so that tools can be built capable of analyzing dependencies between compilation units and possible inconsistencies in the state of the "context". It is the idea that an ASIS context can refer to such a situation in some implementation-dependent way. As defined now, in no case ASIS will provide access to both versions of P, even indirectly by references from A and B. Depending on the ASIS implementation, the compilation unit A may or may not be part of a context. If it is, then ASIS reports that A is obsolete (e.g., by saying that A is inconsistent with P). If issued, queries about A can raise exceptions or produce wrong results. This does not preclude some ASIS implementation providing correct answers to structural queries about A (which are about syntax only).
...
@LabeledSubClause{.4	Abstract "=" for private types }

An Is_Equal function has been defined for the private types Context (Clause 3.5), Element (Clause 3.6), Compilation_Unit (Clause 3.10), Container (Clause 9.1), Line (Clause 20.1), Id (Clause 21.1), Record_Component (Clause 22.1), and Array_Component (Clause 22.3). An abstract function "=" has been defined for each private type because the semantics for the Is_Equal function for these types may differ from the semantics of the default "=" operator. This prevents improper use of the "=" operator.

@LabeledSubClause{.5	Usage names and expressions }
...
@LabeledSubClause{.6	Select alternative }

Let's start by recalling the Ada syntax of a select_alternative (Reference Manual 9.7.1 (4-7)):
...
@LabeledSubClause{.7	Attribute definition clauses }

In Reference Manual 13.3 (2), Ada says that an attribute_definition_clause is defined as:

     attribute_definition_clause ::=
                  for local_name'attribute_designator use expression;
                | for local_name'attribute_designator use name;

In Reference Manual 4.1, Ada says that a name may be an attribute_reference, which then is formally defined in Reference Manual 4.1.4 (2) by:
...
@LabeledSubClause{.9	Queries with additional context parameter }
...
@LabeledSubClause{.11	Data decomposition }

The optional Data_Decomposition package was developed to support the special needs of applications in decoupling system software interfaces. Such interfaces could support communications between two platforms or communications between two subsystems within a system. Typically an interface between two software systems is accomplished through a rigorously defined interface. This rigorous interface is beneficial since it defines the expected communications protocol between the two computers. However, this interface is a serious impediment to upgrading a system when only part of the system is to be upgraded. The new system typically requires a new interface to support the new capabilities. Hence software on both sides of the interface needs to be upgraded to enhance the system.

Data_Decomposition allows the upgrade of part of a critical system without changing software in the rest of the system. Hence it provides significant flexibility for the life-cycle of a system. Decoupling the interface between two software systems can  be accomplished when each system has access to semantic information in the other system's Ada compilation environment. This semantic information describes objects sent across the interface and could serve as a means to identify its operations. Data_Decomposition was specifically developed to support a one-way interface between a United States Air Force satellite and the ground station’s message analysis (delog) processing.
...
@LabeledClause{Acronyms }
	...
	ASISRG		ASIS Rapporteur Group

	ASISWG		ASIS Working Group
