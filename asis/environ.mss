...
@comment{$Revision: 1.1 $ $Date: 2006/09/24 02:39:34 $}
...
With some implementations, Is_Equal may be True before the Contexts
are opened, but may be False after the Contexts are open.
One possible cause for this is a sequence of events such as:

a) ASIS program A opens the Left Context for READ,

b) non-ASIS program B opens the Context for UPDATE, and creates a new
   version of the implementor Context,

c) ASIS program A opens the Right Context for READ, and gets the new version.
...
@LabeledClause{function Parameter }
