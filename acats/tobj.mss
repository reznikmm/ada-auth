...
objectives that it tests. This document provides easier searching for
...
For instance,
@LocalLink{Target=[Ada95],Sec=[References],Text={[Ada95]}} contains the
rule
@begin{Indent}
...
of the rules are tested. Indeed, a number in instances where important rules
...
For these reasons, with ACATS 3.0, the development of new and much more
detailed coverage documentation was started. The new documentation considers
each sentence of the standard individually if necessary, listing the test
objectives for each sentence, and documenting why no objectives are needed
...
the other kinds of tests, especially by the user-oriented tests. Generally,
...
First, rules that
are given text form rather than BNF are treated as legality rules. Second, the
...
checks that too much information is not used for resolution. (limited vs.
non-limited; pool-specific vs. general, etc.) It also may have a C-Test to
