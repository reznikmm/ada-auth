...
@comment($Revision: 1.1 $ $Date: 2006/02/03 07:39:46 $)
...
328  Non-generic version of @exam[Complex_IO

]351  Time operations

362  Some predefined packages should be recategorized

366  More liberal rules for @exam[Pure] units

370  Add standard interface for environment variables

388  Add Greek pi to @exam[Ada.Numerics]
...
427  Default parameters and @exam[Calendar] operations

428  Input@en@;utput for bounded strings
...
@end[Description]
First the container library is rather extensive and merits a whole
paper alone (302). We only refer to it here for completeness.

New child packages of @exam[Calendar] provide extra facilities for
manipulating times and dates (351, 427).
...
operations (248) and environment variables (370).
...
is now support for 16- and 32-bit characters (285, 388, 395, 400),
and there are additional operations in the string packages (301, 428).
...
algebra algorithms (296, 418) and a trivial addition concerning complex
input@en@;utput (328).

The categorization of various predefined units has been changed in
order to remove unnecessary restrictions on their use in distributed
systems and similar applications (362, 366). The new pragma @exam[Preelaborable_Initialization]
is introduced as well for similar reasons (161). We can also group
a minor change to the Distributed Systems annex here (273).

Finally there is new attribute @exam[Stream_Size] in order to increase
the portability of streams (270) and the parameter @exam[Stream] of
@exam[Read], @exam[Write] etc now has a null exclusion (441).
...
The first change to note is that the subtype @exam[Year_Number] in
...
There are three new child packages @exam[Calendar.Time_Zones], @exam[Calendar.Arithmetic]
and @exam[Calendar.Formatting]. We will look at these in turn.
...
@key[package] Ada.Calendar.Time_Zones @key[is]
   ...
   @key[function] UTC_Time_Offset(Date: Time := Clock) @key[return]
Time_Offset;
...
from UTC (which curiously is short for Coordinated Universal Time);
...
an integral number of hours different from UTC.
...
turn up late anyway. The chronophile might find the website {\field\fldedit{\*\fldinst
 HYPERLINK "http://www.merlyn.demon.co.uk/misctime.htm#GMT" {\*\datafield
}}{\fldrslt
www.merlyn.demon.co.uk/misctime.htm# GMT}} of interest.

So the function @exam[UTC_Time_Offset] applied in an Ada program in
...
daylight saving or heure d\rquote \'e9t\'e9). Remember that the type
...
More generally the type @exam[Time_Offset] can be used to represent
...
The package @exam[Calendar.Arithmetic] provides some awkward arithmetic
operations and also covers leap seconds. Its specification is
...
@key[package] Ada.Calendar.Arithmetic @key[is]
   ...
   @key[subtype] Leap_Seconds_Count @key[is] Integer @key[range] @en@;2047
.. 2047;

   @key[procedure] Difference(Left, Right: @key[in] Time;
@\@\@\        Days: @key[out] Day_Count; Seconds: @key[out] Duration;
@\@\@\        Leap_Seconds: @key[out] Leap_Seconds_Count);

   @key[function] "+" (Left: Time; Right: Day_Count) @key[return]
Time;
   @key[function] "+" (Left: Day_Count; Right: Time) @key[return]
Time;
   @key[function] "@en" (Left: Time; Right: Day_Count) @key[return]
Time;
...
a leap second at least four time a year for the foreseeable future
...
seconds makes calculations with times somewhat tricky.
...
Suppose we wanted to find the difference between noon on June 1st
...
However, it should be noted that @exam[Calendar."@en"] counts the
true seconds and so the expression


@begin[Example]
Calendar."@en" (Time_Of(1985, 7, 1, 1*3600.0), Time_Of(1985, 6, 30,
23*3600.0))
...
The third child package @exam[Calendar.Formatting] has a variety of
functions. Its specification is
...
@key[package] Ada.Calendar.Formatting @key[is]
   ...
   @key[type] Day_Name @key[is] (Monday, Tuesday, Wednesday, Thursday,
Friday, Saturday, Sunday);

   {\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Day_Of_Week(Date:
Time) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Day_Name;}
   ...
   @key[subtype] Second_Duration @key[is] Day_Duration @key[range]
0.0 .. 1.0;

   @key[function] Year(Date: Time; Time_Zone: Time_Offset := 0) @key[return]
Year_Number;
...
@\@\Time_Zone: Time_Offset := 0) @key[return] Time;
   ...
   @key[procedure] Split(Date: @key[in] Time;     @\@\--@examcom[
(3)]
@\@\... -- @examcom[as (2) but with additional parameter]
@\@\Leap_Second: @key[out] Boolean;
@\@\Time_Zone: @key[in] Time_Offset := 0);

   @key[procedure] Split(Date: @key[in] Time;    @\@\--@examcom[ (4)]
...
@\@\Time_Zone: @key[in] Time_Offset := 0);
   ...
   @key[function] Image(Date: Time;
@\@\Include_Time_Fraction: Boolean := False;
@\@\Time_Zone: Time_Offset := 0) @key[return] String;

   @key[function] Value(Date: String; Time_Zone: Time_Offset := 0)
@key[return] Time;

   @key[function] Image (Elapsed_Time: Duration;
@\@\Include_Time_Fraction: Boolean := False) @key[return] String;

  {\expnd0\expndtw-2  }{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2
 Value(Elapsed_Time: String) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2
 Duration;
...
Most of these functions have an optional parameter which is a time
...
The function @exam[Seconds_Of] creates a value of type @exam[Duration]
from components @exam[Hour], @exam[Minute], @exam[Second] and @exam[Sub_Second].
Note that we can use this together with @exam[Calendar.Time_Of] to
create a value of type @exam[Time]. For example
...
The first procedure @exam[Split] is the reverse of @exam[Seconds_Of].
...
@end[Example]
The next procedure @exam[Split] (no 2) takes a @exam[Time] and a @exam[Time_Offset]
(optional) and decomposes the time into its seven components. Note
that the optional parameter is last for convenience. The normal rule
for parameters of predefined procedures is that parameters of mode
in are first and parameters of mode out are last. But this is a nuisance
if parameters of mode in have defaults since this forces named notation
...
The purpose of this parameter needs to be understood carefully. Making
up a typical time will have this parameter as @exam[False]. But suppose
...
@end[Example]
In a sense there were two 19:59:59 that day in New York. The proper
...
Note that if the parameter @exam[Leap_Second] is @exam[True] and the
other parameters do not identify a time of a leap second  then @exam[Time_Error]
is raised.

There are then two corresponding procedures @exam[Split] (nos 3 and
...
@end[Example]
results in @exam[Leap] being @exam[True] whereas
...
On the other hand calling the version of @exam[Split] (no 2) without
...
Finally, there are two pairs of functions @exam[Image] and @exam[Value].
...
@end[Example]
If we set the optional parameter @exam[Include_Time_Fraction] to @exam[True]
thus
...
@end[Example]
There is also the usual optional @exam[Time_Zone] parameter so we
...
We would expect to get exactly the same results with @exam[Magic_Moment].
...
@end[Example]
The other @exam[Image] and @exam[Value] pair work on values of type
@exam[Duration] thus
...
The package @exam[Ada.Environment_Variables] has the following specification
...
@key[package] Ada.Environment_Variables @key[is]
   ...
   @key[procedure] Iterate(Process: @key[not null access procedure]
...
The behaviour is straightforward. We might check to see if there is
...
We can iterate over the variables using the procedure @exam[Iterate].
...
The other environment package is @exam[Ada.Directories]. Its specification
is
...
@key[package] Ada.Directories @key[is]
   ...
   @key[procedure] Create_Directory(New_Directory: @key[in ]String;
Form: @key[in ]String := "");
   @key[procedure] Delete_Directory(Directory: @key[in ]String);
   @key[procedure] Create_Path(New_Directory: @key[in ]String; Form:
@key[in ]String := "");
   @key[procedure] Delete_Tree(Directory: @key[in ]String);
   @key[procedure] Delete_File(Name: @key[in ]String);
   @key[procedure] Rename(Old_Name: @key[in] String; New_Name: @key[in
]String);
   @key[procedure]{\expnd0\expndtw-2  Copy_File(Source_Name: }{\cs20\b\expnd0\expndtw-2
in}{\expnd0\expndtw-2  String; Target_Name: }{\cs20\b\expnd0\expndtw-2
in }{\expnd0\expndtw-2 String; Form: }{\cs20\b\expnd0\expndtw-2 in
}{\expnd0\expndtw-2 String := "");
   ...
   @key[function] Containing_Directory(Name: String) @key[return]
String;
   @key[function] Extension(Name: String) @key[return] String;
   @key[function] Base_Name(Name: String) @key[return] String;
   @key[function] Compose(Containing_Directory: String := ""; Name:
...
@\@\@\@\@\@\@\@\@\@\        @key[return] String;
   ...
   @key[procedure] Start_Search(Search: @key[in out] Search_Type;
...
@\@\@\Filter: @key[in ]Filter_Type := (@key[others] => True));
   ...
   @key[procedure] Get_Next_Entry(Search: @key[in out] Search_Type;
@\@\@\Directory_Entry: @key[out] Directory_Entry_Type);
   @key[procedure] Search(Directory: @key[in] String;
...
@\@\           (Directory_Entry: @key[in] Directory_Entry_Type));
   ...
   @key[function] Simple_Name(Directory_Entry: Directory_Entry_Type)
@key[return] String;
   @key[function] Full_Name(Directory_Entry: Directory_Entry_Type)
@key[return] String;
   @key[function] Kind(Directory_Entry: Directory_Entry_Type) @key[return]
File_Kind;
   @key[function] Size(Directory_Entry: Directory_Entry_Type) @key[return]
File_Size;
...
Unix; these cannot be created or read by the predefined Ada input@en@;utput
packages.
...
@end[Example]
For the sake of illustration we will continue with the simple DOS
...
@end[Example]
then the function @exam[Current_Directory] will return the string
...
The procedure @exam[Create_Path] creates several nested directories
...
used to copy any file that could be copied using a normal input@en@;utput
...
similar parameters in the predefined input@en@;utput packages the meaning
...
The function @exam[Full_Name] returns the full name of a file. Thus
assuming the current directory is still @exam["c:\adastuff"]
...
@end[Example]
returns @exam["c:\adastuff\rat\library.doc"] and
...
@end[Example]
returns @exam["library.doc"] and not @exam["rat\library.doc"]. We
can also apply @exam[Simple_Name] to a string that does not go back
to the root. Thus


@begin[Example]
Simple_Name({\cs16 "rat\library.doc");

}
@end[Example]
is allowed and also returns@exam[ "library.doc"].

The function @exam[Containing_Directory_Name] removes the simple name
...
@end[Example]
and this returns @exam["..\rat"]; note that it also removes the separator@exam[
"\"].

The functions @exam[Extension] and @exam[Base_Name] return the corresponding
parts of a file name thus
...
The function @exam[Compose] can be used to put the various bits together,
thus
...
@end[Example]
returns @exam["rat\library.doc"]. The default parameters enable bits
...
is measured in stream elements @en usually bytes.)

Various types and subprograms are provided to support searching over
...
We will look at the active iterator approach first.

The procedures @exam[Start_Search], @exam[End_Search] and @exam[Get_Next_Entry]
and the function @exam[More_Entries] control the search loop. The
general pattern is
...
Suppose we want to look for all ordinary files with extension @exam["doc"]
in the directory @exam["c:\adastuff\rat"]. We could write
...
...
{\expnd0\expndtw-2 Start_Search(Rat_Search, "c:\adastuff\rat", "*.doc",
Filter);}
...
The alternative mechanism using a passive iterator is as follows.
...
@end[Example]
and then declare a filter and call the procedure @exam[Search] thus
...
So to complete this example we might print out a table of the files
...
@end[Example]
Note that the function @exam[Image] is from the package @exam[Ada.Calendar.Formatting]
...
@begin[Example]
{\f3\lang1033\langfe1033\langnp1033 \'70}{\lang1033\langfe1033\langnp1033
: }@key[constant]{\lang1033\langfe1033\langnp1033  := Pi;

}
@end[Example]
{\lang1033\langfe1033\langnp1033 to the package }@exam[Ada.Numerics]{\lang1033\langfe1033\langnp1033
. This enables us to write mathematical programs in a more natural
notation thus

}
@begin[Example]
Circumference: Float := 2.0 * {\f3\lang1033\langfe1033\langnp1033
\'70} * Radius;
...
@begin[Example]
Part := Delete(BS, I, I+2);@\@\@\-- @examcom[2 is length "bar" @en
1]
...
(we have added a use clause for brevity as usual)
...
with the maximum string size. So the specification is
...
4@\a complex input@en@;utput package.
...
:= 1) @key[return] Real_Matrix;@examcom[
...
There are two functions @exam[Solve] for solving one and several sets
of linear equations respectively. Thus if we have the single set of
@i[n] equations
...
@end[Example]
and if we have @i[m] sets of @i[n] equations we might write
...
@end[Example]
The functions @exam[Inverse] and @exam[Determinant] are provided for
...
determinant will be of the order of 10{\super 30}. The user may therefore
have to scale the data.

Two subprograms are provided for determining the eigenvalues and eigenvectors
...
The reader will observe that the facilities provided here are rather
...
@begin[Description]
{\u9642\'3f@\to provide commonly required simple facilities for the
user who is not a numerical professional,

\u9642\'3f@\to provide a baseline of types and operations that forms
a firm foundation for binding to more general facilities such as the
BLAS.
...
The complex package is very similar and will not be described in detail.
However, the generic formal parameters are interesting. They are
...
@end[Example]
Thus we see that it has two formal packages which are the corresponding
...
have real eigenvalues and are well behaved.

We conclude this discussion of the Numerics annex by mentioning one
minute change regarding complex input@en@;utput. Ada 2005 includes
preinstantiated forms of @exam[Ada.Text_IO.Complex_IO] such as @exam[Ada.Complex_Text_IO]
(for when the underlying real type is the type @exam[Float]), @exam[Ada.Long_
Complex_Text_IO] (for type @exam[Long_Float]) and so on. These are
by analogy with @exam[Float_Text_IO], @exam[Long_Float_Text_IO] and
...
It will be recalled that library units in Ada 95 are categorized into
...
The pragmas @exam[Shared_Passive], @exam[Remote_Types], and @exam[Remote_Call_Interface]
concern distributed systems and thus are rather specialized. A minor
change made in the 2001 Corrigendum was that the pragma @exam[Remote_Types]
was added to the package @exam[Ada.Finalization ]in order to support
the interchange of controlled types between partitions in a distributed
system.

Note that the pragma @exam[Preelaborate] does not fit into this hierarchy.
...
The following packages which had no categorization in Ada 95 have
...
@end[Example]
The following which had pragma @exam[Preelaborate] in Ada 1995 have
...
Just for the record the following packages (and functions, @exam[Hash]
...
@end[Example]
And the following new packages and functions have the pragma @exam[Preelaborate]
...
A problem with preelaborable units in Ada 95 is that there are restrictions
on declaring default initialized objects in a unit with the pragma
@exam[Preelaborate]. For example, we cannot declare objects of a private
type at the library level in such a unit. This is foolish for consider
...
@end[Example]
Clearly these declarations can be preelaborated and so the package
...
This is overcome in Ada 2005 by the introduction of the pragma @exam[Preelaborable_Initialization].
Its syntax is
...
The following predefined private types which existed in Ada 95 have
...
that it was not possible to apply the pragma to@exam[ Ada.Strings.Bounded.Generic_Bounded_Length.Bounded_String]
because it would have made it impossible to instantiate @exam[Generic_Bounded_Length]
with a non-static expression for the parameter @exam[Max].

The following private types which are new in Ada 2005 also have the
