...
@leading@keepnext@;The root package is
@begin[Example]
...
We will first consider the list container since in some ways it is
the simplest. Here is its specification interspersed with some explanation
...
A list container is an object of the type @exam[List]. It is tagged
...
Note that @exam[A_List = Empty_List], @exam[Is_Empty(A_List)] and@exam[
Length(A_List) = 0] all have the same value.


@begin[Example]
@key[function]{\expnd0\expndtw-2  Element(Position: Cursor) }{\cs20\b\expnd0\expndtw-2
return}{\expnd0\expndtw-2  Element_Type;}

@key[procedure] Replace_Element(Container:@key[ in out ]List; Position:
@key[in] Cursor;
...
@begin[Example]
@key[procedure] Query_Element(Position: @key[in] Cursor;
@\@\@\Process: @key[not null access procedure] (Element: @key[in]
Element_Type));

@key[procedure] Update_Element(Container: @key[in out] List; Position:
@key[in] Cursor;
...
@key[procedure] Delete_First(Container: @key[in out] List; Count:
@key[in] Count_Type := 1);

@key[procedure] Delete_Last(Container: @key[in out] List; Count: @key[in]
Count_Type := 1);
...
@begin[Example]
@key[procedure] Swap(Container:@key[ in out] List; I, J: @key[in]
Cursor);
...
@begin[Example]
@key[procedure] Swap_Links(Container: @key[in out ]List; I, J: @key[in]
Cursor);
...
There is no analogy in the vectors package.
...
@key[procedure] Splice(Container: @key[in out] List;
@\@\@\Before: @key[in] Cursor;
@\@\@\Position: @key[in out] Cursor);
...
package because, like {\cs40\f1\fs18 Swap_Links}, we are just moving
...
@key[function] Contains(Container: List; Item: Element_Type) @key[return]
Boolean;
...
@begin[Example]
{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Has_Element(Position:
Cursor) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Boolean;
...
@begin[Example]
@key[procedure] Iterate(Container: @key[in] List;
@\@\@\Process: @key[not null access procedure] (Position: @key[in]
Cursor));

@key[procedure] Reverse_Iterate(Container: @key[in] List;
@\@\@\Process: @key[not null access procedure] (Position: @key[in]
Cursor));
...
@end[Example]
If the reader has got this far they have probably understood how to
...
This especially applies to the procedures @exam[Process] in @exam[Query_Element],
...
In fact the vectors package enables us to access elements either by
using an index or by using a cursor. Thus many operations are duplicated
such as


@begin[Example]
@key[function] Element(Container: Vector; Index: Index_Type) @key[return]
Element_Type;
@key[function]{\expnd0\expndtw-2  Element(Position: Cursor) }{\cs20\b\expnd0\expndtw-2
return}{\expnd0\expndtw-2  Element_Type;

}@key[procedure] Replace_Element(Container: @key[in out] Vector;
...
There are also functions @exam[First_Index] and @exam[Last_Index]
thus
...
respectively. The function @exam[First_Index] always returns @exam[Index_Type'First]
whereas @exam[Last_Index] will return @exam[No_Index] if the vector
is empty. The function @exam[Length] returns @exam[Last_Index@en@;First_Index+1]
which is zero if the vector is empty. Note that the irritating subtype
@exam[Extended_Index] has to be introduced in order to cope with end
values. The constant @exam[No_Index] has the value @exam[Extended_Index'First]
which is equal to @exam[Index_Type'First@en@;1].

There are operations to convert between an index and a cursor thus


@begin[Example]
@key[function] To_Cursor(Container: Vector; Index: Extended_Index)
@key[return] Cursor;
...
@key[package body] Stack is
   ...
   @key[package] Float_Container @key[is] @key[new] Vectors(Natural,
Float);@\-- @examcom[changed]
   @key[use] Float_Container;
...
There are other important properties of vectors that should be mentioned.
One is that there is a concept of capacity. Vectors are adjustable
...
@begin[Example]
@key[procedure] Reserve_Capacity(Container: @key[in out] Vector; Capacity:
@key[in] Count_Type);
...
@begin[Example]
{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Capacity(Container:
Vector) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Count_Type;
...
There is also the concept of "empty elements". These are elements
...
@begin[Example]
@key[function] To_Vector(New_Item: Element_Type; Length: Count_Type)
@key[return] Vector;
...
@begin[Example]
@key[procedure] Set_Length(Container:@key[ in out] Vector; Length:
@key[in] Count_Type);
...
There are further versions of @exam[Insert] thus
...
@key[procedure] Insert(Container: @key[in out] Vector;
@\@\@\Before: @key[in] Cursor; New_Item: @key[in] Vector; Position:
@key[out] Cursor);
...
There are also corresponding versions of @exam[Prepend] and @exam[Append]
thus


@begin[Example]
@key[procedure] Prepend(Container: @key[in out] Vector; New_Item:
@key[in] Vector);

@key[procedure] Append(Container: @key[in out] Vector; New_Item: @key[in]
Vector);


@end[Example]
Finally, there are four functions @exam["&"] which concatenate vectors
and elements by analogy with those for the type @exam[String]. Their
specifications are


@begin[Example]
@key[function] "&" (Left, Right: Vector) @key[return] Vector;@key[

function] "&" (Left: Vector; Right: Element_Type) @key[return] Vector;@key[
function] "&" (Left: Element_Type; Right: Vector) @key[return] Vector;
...
which are a little simpler and then consider hashed maps.

Remember that a map is just a means of getting from a value of one
...
There are two maps packages with much in common. One keeps the keys
...
@key[package] Ada.Containers.Ordered_Maps @key[is]
   ...
   @key[function] Equivalent_Keys(Left: Right: Key_Type) @key[return]
Boolean;
...
For the convenience of the user the function @exam[Equivalent_Keys]
...
@begin[Example]
@key[function]{\expnd0\expndtw-2  Key(Position: Cursor) }{\cs20\b\expnd0\expndtw-2
return}{\expnd0\expndtw-2  Key_Type;}@key[
function]{\expnd0\expndtw-2  Element(Position: Cursor) }{\cs20\b\expnd0\expndtw-2
return}{\expnd0\expndtw-2  Element_Type;}

@key[procedure] Replace_Element(Container:@key[ in out] Map;
@\@\@\Position: @key[in] Cursor;
@\@\@\New_Item: @key[in] Element_Type);
...
@key[procedure] Update_Element(Container: @key[in out] Map; Position:
@key[in] Cursor;
@\Process: @key[not null access procedure] (Key: @key[in] Key_Type;
...
For example the key part might be a record with part number and year
...
@end[Example]
and we might define the ordering relationship to be used as the generic
parameter simply in terms of the part number
...
@begin[Example]
@key[procedure] Exclude(Container: @key[in out] Map; Key: @key[in]
Key_Type);
...
@begin[Example]
@key[procedure] Delete(Container: @key[in out] Map; Key: @key[in]
Key_Type);

@key[procedure] Delete(Container: @key[in out] Map; Position: @key[in
out] Cursor);
...
@key[function] Element(Container: Map; Key: Key_Type) @key[return]
Element;
@key[function] Contains(Container: Map; Key: Key_Type) @key[return]
Boolean;
...
}@key[procedure] Iterate(Container: @key[in] Map;
...
As an example we can make a container to hold the information concerning
spare parts. We can use the type @exam[Part_Key] and the function
...
@\@key[new] Ordered_Maps(Key_Type => Part_Key,
...
@begin[Example]
@key[procedure] Request(Part: @key[in] Integer; OK: @key[out] Boolean;
@\@\              Year: @key[out] Integer; Shelf: @key[out] Character)
@key[is
]   C: Cursor;
   K: Part_Key;
   E: Stock_Info;@key[
begin
   ...
   @key[if] C = No_Element @key[then]
      OK := False; @key[return];@\@\@\--@i[ ]@examcom[no such key]
   @key[end if];
   E := Element(C);  K := Key(C);
...
@end[Example]
Note that we had to put a dummy year number in the call of @exam[Find].
We could of course use the new @exam[<>] notation for this
...
@end[Example]
The reader can improve this example at leisure @en by using @exam[Update_Element]
for example.

As another example suppose we wish to check all through the stock
...
Another approach is to use @exam[First] and @exam[Next] and so on
thus
...
Defining good hash functions needs care. In the case of the part numbers
we might multiply the part number by some obscure prime number and
...
golden number {\f3 \'74} = ({\f3 \'d6}5+1)/2 = 1.618... and then taking
...
monk who lived in Paris. He studied numbers of the form @i[M]{\i\sub
p} = 2{\i\super p} @en 1 where @i[p] is prime. A lot of these are
themselves prime. Mersenne gave a list of those upto 257 which he
...
largest which is also the largest known prime number is @i[M]{\sub
25964951} @en see www.mersenne.org.

The specification of the hashed maps package is very similar to that
for ordered maps. It starts
...
@begin[Example]
@key[function] Equivalent_Parts(Left, Right: Part_Key) @key[return]
Boolean @key[is]
@key[begin]
   @key[return] Left.Part_Number = Right.Part_Number;
...
@begin[Example]
@key[package] Store_Maps @key[is
@\new] Hashed_Maps(Key_Type => Part_Key,
@\@\@\Element_Type => Stock_Info,
@\@\@\Hash => Part_Hash,
...
@key[function] Ceiling(Container: Map; Key: Key_Type) @key[return]
Cursor;
...
@key[procedure] Reverse_Iterate(Container: @key[in] Map;
@\@\Process: @key[not null access procedure] (Position: @key[in] Cursor));
...
The functions @exam["<"] and @exam[">" ]are mostly for convenience.
...
Hashed maps have a very important facility not in ordered maps which
...
@begin[Example]
@key[procedure] Reserve_Capacity(Container: @key[in out] Map; Capacity:
@key[in] Count_Type);

{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Capacity(Container:
Map) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Count_Type;
...
@key[function] Equivalent_Keys(Left: Cursor; Right: Key_Type) @key[return]
Boolean;
@key[function] Equivalent_Keys(Left: Key_Type; Right: Cursor) @key[return]
Boolean;


@end[Example]
These (like the additional @exam["<"] and @exam[">"] for ordered maps)
...
@end[Example]
Before moving on to sets it should be noticed that there are also
some useful functions in the string packages. The main one is
...
We could for example save ourselves the worry of defining a good hash
...
those facilities that are common to both kinds of sets.
...
@key[package] Ada.Containers.Ordered_Sets @key[is]
   ...
   @key[function] Equivalent_Elements(Left, Right: Element_Type) @key[return]
Boolean;
...
elements is only used by the function @exam["="] for comparing two
sets.
...
@key[function]{\expnd0\expndtw-4  Equivalent_Sets(Left, Right: Set)
}{\cs20\b\expnd0\expndtw-4 return}{\expnd0\expndtw-4  Boolean;
}@key[function]{\expnd0\expndtw-4  To_Set(New_Item: Element_Type)
}@key[return]{\expnd0\expndtw-4  Set;}
...
@key[procedure] Replace_Element(Container: @key[in out ]Set;
@\@\@\Position: @key[in] Cursor;
@\@\@\New_Item: @key[in] Element_Type);
...
@begin[Example]
@key[procedure] Include(Container: @key[in out] Set; New_Item: @key[in]
Element_Type);
...
@begin[Example]
@key[procedure] Replace(Container: @key[in out] Set; New_Item: @key[in]
Element_Type);
...
@begin[Example]
@key[procedure] Exclude(Container: @key[in out] Set; Item: @key[in]
Element_Type);
...
@begin[Example]
@key[procedure] Delete(Container: @key[in out] Set; Item: @key[in]
Element_Type);

@key[procedure] Delete(Container: @key[in out] Set; Position: @key[in
out] Cursor);
...
@key[function] "or" (Left, Right: Set) @key[return] Set @key[renames]
Union;

@key[procedure] Intersection(Target: @key[in out ]Set; Source: @key[in]
Set);
@key[function] Intersection(Left, Right: Set) @key[return] Set;
@key[function] "@key[and]" (Left, Right: Set) @key[return] Set @key[renames]
Intersection;

@key[procedure] Difference(Target: @key[in out] Set; Source: @key[in]
Set);
@key[function] Difference(Left, Right: Set) @key[return] Set;
@key[function] "@en" (Left, Right: Set) @key[return] Set @key[renames]
Difference;

@key[procedure] Symmetric_Difference(Target: @key[in out] Set; Source:
@key[in] Set);
@key[function] Symmetric_Difference (Left, Right: Set) @key[return]
Set;
@key[function] "@key[xor]" (Left, Right: Set) @key[return] Set @key[renames]
...
@key[function] Next(Position: Cursor) @key[return] Cursor;
@key[procedure] Next(Position:@key[ in out] Cursor);
@key[function] Find(Container: Set; Item: Element_Type) @key[return]
Cursor;
@key[function] Contains(Container: Set; Item: Element_Type) @key[return]
Boolean;
...
}@key[procedure] Iterate(Container: @key[in] Set;
@\@\Process: @key[not null access procedure] (Position: @key[in] Cursor));
...
@key[generic]
   ...
   @key[with function] "<" (Left, Right: Key_Type)@\ @key[return]
Boolean @key[is] <>;
...
@key[function] Element(Container: Set; Key: Key_Type) @key[return]
Element_Type;

@key[procedure] Replace(Container: @key[in out] Set;
@\@\Key: @key[in] Key_Type; New_Item: @key[in] Element_Type);

@key[procedure] Exclude(Container: @key[in out] Set; Key: @key[in]
Key_Type);
@key[procedure] Delete(Container: @key[in out] Set; Key: @key[in]
Key_Type);

@key[function] Find(Container: Set; Key: Key_Type) @key[return] Cursor;
@key[function] Contains(Container: Set; Key: Key_Type) @key[return]
Boolean;
...
However, sets can be used as sort of economy class maps by using the
...
We then declare @exam["<"] much as before
...
@begin[Example]
@key[package] Store_Sets @key[is] @key[new] Ordered_Sets(Element_Type
=> Part_Type);
...
@key[begin
   ...
   @key[if] C = No_Element @key[then]
      OK := False; @key[return];@\@\@\--@i[ ]@examcom[no such item]
   @key[end if];
   E := Element(C);
   Year := E.Year;
   Shelf := E.Shelf;
...
@begin[Example]
@key[package] Party @key[is] @key[new] Generic_Keys(Key_Type => Part_Key,
Key => Part_No);
@key[use] Party;
...
And now we can rewrite the @exam[Request] procedure as follows


@begin[Example]
@key[procedure] Request(Part: @key[in] Part_Key; OK: @key[out] Boolean;
@\              Year: @key[out] Integer; Shelf: @key[out] Character)
@key[is
]   C: Cursor;
   E: Part_Type;@key[
...
with the ordering used to instantiate the parent package @exam[Containers.Ordered_Sets]
itself. If this is not the case then the sky might fall in.

Incidentally, the procedure for checking the stock which previously
...
@end[Example]
The only change is that the call of @exam[Key] in
...
@end[Example]
when using the maps package has been replaced by @exam[Element]. A
...
@begin[Example]
@key[function] Equivalent_Parts(Left, Right: Part_Type) @key[return]
Boolean @key[is]
@key[begin]
   @key[return] Left.Part_Number = Right.Part_Number;
@key[end] Equivalent_Parts;
...
@key[function] Part_Hash(P: Part_Type) @key[return] Hash_Type @key[is]
   M31: @key[constant] := 2**31@en@;1;    @\@\--@examcom[ a nice Mersenne
prime]
@key[begin]
   @key[return] Hash_Type(P.Part_Number) * M31;
@key[end] Part_Hash;
...
@end[Example]
(which are very similar to the hashed map example @en the only changes
are to the parameter type name) then we can instantiate the hashed
sets package as follows


@begin[Example]
@key[package] Store_Sets @key[is]
@\@key[new] Hashed_Sets(Element_Type => Part_Type,
@\@\@\Hash => Part_Hash,
...
It should also be mentioned that the inner package @exam[Generic_Keys]
for hashed sets has the following formal parameters
...
Although our example itself is unchanged we do have to change the
...
@begin[Example]
@key[package] Party @key[is]
@\@key[new] Generic_Key(Key_Type => Part_Key,
@\@\@\Key => Part_No;
@\@\@\Hash => Part_Hash
@\@\@\Equivalent_Keys => Equivalent_Parts);
@key[use] Party;
...
@key[function] Floor(Container: Set; Item: Element_Type) @key[return]
Cursor;
@key[function] Ceiling(Container: Set; Item: Element_Type) @key[return]
Cursor;

@key[function] "<" (Left, Right: Cursor) @key[return] Boolean;
@key[function] ">" (Left, Right: Cursor) @key[return] Boolean;
@key[function] "<" (Left: Cursor; Right: Element_Type) @key[return]
Boolean;
@key[function] ">" (Left: Cursor; Right: Element_Type) @key[return]
Boolean;
@key[function] "<" (Left: Element_Type; Right: Cursor) @key[return]
Boolean;
@key[function] ">" (Left: Element_Type; Right: Cursor) @key[return]
Boolean;

@key[procedure] Reverse_Iterate(Container: @key[in] Set;
@\@\Process: @key[not null access procedure] (Position: @key[in] Cursor));
...
than @exam[Item] @en they return @exam[No_Element] if there is not
one.

The functions @exam["<"] and @exam[">"] are very important for ordered
sets. The first is equivalent to
...
The following are additional in the package @exam[Generic_Keys] for
ordered sets
...
@key[function] Ceiling(Container: Set; Key: Key_Type) @key[return]
Cursor;
...
Hashed sets, like hashed maps also have the facility to specify a
capacity as for the vectors package. Thus we have


@begin[Example]
@key[procedure] Reserve_Capacity(Container: @key[in out] Set; Capacity:
@key[in] Count_Type);

{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Capacity(Container:
Set) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Count_Type;
...
@begin[Example]
@key[function] Equivalent_Elements(Left, Right: Cursor) @key[return]
Boolean;
@key[function] Equivalent_Elements(Left: Cursor; Right: Element_Type)
@key[return] Boolean;
@key[function] Equivalent_Elements(Left: Element_Type; Right: Cursor)
@key[return] Boolean;


@end[Example]
Again, these are very important for sets. The first is equivalent
to
...
As mentioned in Section 1, an indefinite (sub)type is one for which
...
wide types are also indefinite.
...
@key[generic]
   ...
   @key[with] @key[function] "=" (Left, Right: Element_Type) @key[return]
Boolean @key[is] <>;
{\cs20\b\expnd0\expndtw-4 package}{\expnd0\expndtw-4  Ada.Containers.Indefinite_Doubly_Linked_Lists
}{\cs20\b\expnd0\expndtw-4 is}
   @key[pragma] Preelaborate(Indefinite_Doubly_Linked_Lists);


@end[Example]
where we see that the formal type @exam[Element_Type] has unknown
...
@exam[Process] of @exam[Update_Element] (or the garrulous @exam[Update_Element_Preserving_Key]
in the case of sets) can be constrained even if the type @exam[Element_Type]
is unconstrained.

As an example of the use of an indefinite container consider the problem
of creating an index. For each word in a text file we need a list
...
We can suppose that the positions of the words are described by a
...
@end[Example]
The index is essentially a map from the type @exam[String] to a list
of values of type @exam[Place]. We first create a definite list container
for handling the lists thus


@begin[Example]
@key[package] Places@key[ is new ]Doubly_Linked_Lists(Place);{\expnd0\expndtw-2


}
@end[Example]
We then create an indefinite map container from the type @exam[String]
...
It might be helpful for the index to contain information saying which
file it refers to. We can extend the type @exam[Map] thus (remember
...
@end[Example]
We can now declare various subprograms to manipulate our map. For
...
@begin[Example]
@key[procedure] Add_Entry(Index: @key[in out] Text_Map; Word: String;
P: Place) @key[is]
...
A big problem with the procedure as written however is that it uses
...
@key[begin]
   ...
   @key[else]
      -- @examcom[it's an old word
         ...
         @key[procedure] Add_It(The_Key: @key[in] String; The_List:
@key[in out] Places.List) @key[is]
         @key[begin]
            The_List.Append(P);
         @key[end] Add_It;
      @key[begin]
...
@end[Example]
This is still somewhat untidy. In the case of a new word we might
...
@key[begin]
   @key[if] M_Cursor = Indexes.No_Element @key[then]
      --@examcom[ it's a new word
]      {\expnd0\expndtw-4 Index.Insert(Word, Places.Empty_List, M_Cursor,
OK);}
   ...
   @key[declare
      ...
      @key[procedure] Add_It(The_Key: @key[in] String; The_List: @key[in
out] Places.List) @key[is]
      @key[begin]
         The_List.Append(P);
      @key[end] Add_It;
   @key[begin]
...
P: Place) @key[is]
   ...
   Inserted: Boolean;
@key[begin]
   {\expnd0\expndtw-4 Index.Insert(Word, M_Cursor, Inserted);}
...
We can now do some queries on the index. For example we might want
...
@begin[Example]
{\cs20\b\expnd0\expndtw-2 function}{\expnd0\expndtw-2  Four_Letters(Index:
Text_Map) }{\cs20\b\expnd0\expndtw-2 return}{\expnd0\expndtw-2  Integer
}{\cs20\b\expnd0\expndtw-2 is}{\expnd0\expndtw-2
}   Count: Integer := 0;
...
@end[Example]
We might finally wish to know how many four-lettered words there are
...
@begin[Example]
@key[function] Four_Letters_On_Page(Index: Text_Map;
